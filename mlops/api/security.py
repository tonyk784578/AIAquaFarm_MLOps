"""HTTP hardening middlewares for the MLOps API.

Sibling of ``backend/app/core/security_middleware.py`` and
``agents/runtime/security.py`` — same code, replicated because the three
services are packaged independently. Keep all three in sync.
"""
